...
uncommitted) into a private shadow ref ``refs/ros/snapshots/<run_id>`` WITHOUT
...
def capture_git_snapshot(run_id: str, cwd: str | None = None) -> dict[str, Any]:
    """Capture full working state into ``refs/ros/snapshots/<run_id>``.
    ...
    tmp = tempfile.NamedTemporaryFile(prefix="ros-index-", delete=False)
    tmp.close()
    index_file = tmp.name
    try:
