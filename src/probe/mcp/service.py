...
_VIEWS: dict[tuple[str, str], str] = {
    ...
    # The wiki's page index. Reachable from the same ref as the front page,
    # because an agent asking "what else is written down" is still asking about
    # the wiki.
    (EntityType.WIKI, View.PAGES): "_view_wiki_pages",
}
...
class ResearchReadService:
    ...
    def _view_wiki_card(self, entity: dict, request: _Req) -> _ViewData:
        ...
        if entity.get("body"):
            return _ViewData()
        pages = entity.get("page_count")
        if pages:
            # The distinction that decides what the agent does next. A blank
            # front page with pages behind it is a wiki worth reading -- the
            # nightly run just has not written the overview yet -- and an agent
            # told only "empty" would stop here and re-derive everything those
            # pages already say.
            return _ViewData(
                payload={
                    "note": (
                        f"this team's wiki has {pages} page(s) but no front page "
                        "yet (the nightly synthesis run writes it). The pages are "
                        'real: get_entity(ref="wiki", view="pages") lists them and '
                        'get_entity(ref="wiki:<type>/<slug>") reads one.'
                    )
                }
            )
        return _ViewData(
            payload={
                "note": (
                    "this team has no wiki pages yet. They are written by a "
                    "nightly synthesis run and by hand with "
                    "`probe wiki write <type>/<slug>`; an empty wiki is not a "
                    "failed read."
                )
            }
        )

    def _view_wiki_pages(self, entity: dict, request: _Req) -> _ViewData:
        """Every page in the wiki: type, slug, title, when it last moved.

        The INDEX of the wiki rather than its contents, and the reason it is a
        view rather than a second tool: an agent that has read the front page
        and wants the page behind one sentence of it is still asking about the
        same entity, and a `research_list_wiki_pages` tool would put that one
        hop behind a name it would have to already know.

        `entity` is overridden for the reason `_view_wiki_versions` overrides
        it: the resolved wiki entity carries the front page's whole body, and
        letting it ride along on the one view designed to avoid bodies would
        undo the point at the last step.
        """
        payload = self.source.list_wiki_pages()
