...
The plugin's `.mcp.json` reads `${PROBE_MCP_TOKEN}`, so it must be present in the environment
...
connects automatically once `PROBE_MCP_TOKEN` is set. **Restart the session** so the loader
...
#       "headers": { "Authorization": "Bearer ${PROBE_MCP_TOKEN}" } } } }
#
# (b) Local stdio — works with any `claude mcp add`:
claude mcp remove probe-research -s user 2>/dev/null   # idempotent re-add
claude mcp add -s user -e PROBE_MCP_TOKEN="$PROBE_MCP_TOKEN" \
  probe-research -- "$(command -v probe-research-mcp)"     # absolute path: claude's spawn env may lack ~/.local/bin
```
...
- Tokens land in plaintext in `~/.config/probe/config.json` (chmod `600`) and, if you pass
  `-e PROBE_MCP_TOKEN=…` to `claude mcp add`, in `~/.claude.json`. Never echo tokens into
