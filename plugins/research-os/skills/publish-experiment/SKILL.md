...
4. Pin any approved reusable results as asset versions from their run artifacts: `exp asset version ASSET_ID --from-artifact ARTIFACT_ID --label LABEL`.
5. Mint the immutable experiment version manifest: `exp version create EXPERIMENT_ID --label LABEL`. Report the created version.
