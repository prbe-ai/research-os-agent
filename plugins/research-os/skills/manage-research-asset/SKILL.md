...
2. If an exact compatible version exists, materialize its pinned bytes with `exp asset materialize NAME --to PATH`. Record consumption; do not copy it into a new identity.
3. If the purpose is the same but the content must change, produce the new content, upload it as a run artifact (`exp artifact add RUN PATH`), then pin it as a new version of the SAME asset: `exp asset version ASSET_ID --from-artifact ARTIFACT_ID --label LABEL`. Use `research_compare` to show the diff/compatibility impact.
4. If no compatible identity exists, register a new asset (`exp asset register NAME --kind KIND`) and pin its first version. Only start a new identity when resolution returned no compatible match; record the concrete reason in the experiment.
