...
| `daemon` | yes | the daemon | You launch, the daemon records. Yours: create the project, experiment and sweep group you launch into, start runs (`probe exec` or the SDK), the run's own data (metrics, spans, trials, files the run itself attaches) and `run end`. The daemon's: everything else (notes, artifacts, papers, tags, names, descriptions, lineage). If the researcher asks for one of the daemon's writes, add `--directed`. If status says `daemon (degraded)`, the daemon is down: record as in `on`. |
