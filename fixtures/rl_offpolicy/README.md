...
replay buffer on a chain-walk MDP), instrumented with the `probe` SDK the way we'd
...
¹ `env_ref` / `foreign_keys` land on the real columns only against Probe Research **with
...
## Run it against a live Probe Research
...
#   or: export PROBE_BASE_URL=... PROBE_TOKEN=probe_pat_...
python -m fixtures.rl_offpolicy.train
```

Prints the run id + petname. Then in the dashboard (or via `probe bundle <run>`) you'll
see the metric series climbing, the rollout spans, and the artifacts.

Verified end-to-end: the RL run drives a live Probe Research on real Postgres and every
tracked series/span/artifact reads back (see `Probe Research` `tests/integration/test_rl_fixture_lands.py`).
